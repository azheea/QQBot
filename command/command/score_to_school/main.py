...
import os
import command.command.score_to_school.GetScore as GetScore
...
# --------------------------------------
def main(province_id,year,want):
    path = f"{os.getcwd()}/command/command/score_to_school/"  # 获取当前工作目录的路径
    print(path)
        ...
        if not province_id.isdigit() or province_id == "None":  # 如果province_id不是数字或为字符串"None"
            ...
            for key, value in all_province['data'].items():  # 遍历省份信息字典
                if value['provinceName'] == province_id:  # 如果省份名称与输入的省份名称相同
                    province_id = key  # 获取省份ID
                    break

            else:
                return "找不到该省份的ID"  # 打印找不到省份ID的提示信息
    ...
    def get_school_ids():
        all_school_id = {}
        with open(".\command\command\score_to_school\schoolid.json", "r", encoding="utf-8") as f:
